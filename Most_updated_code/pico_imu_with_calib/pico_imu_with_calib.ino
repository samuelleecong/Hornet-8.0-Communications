...
void loop()
{
  ...
  float accel_x = acc_calibrated[0];
  float accel_y = acc_calibrated[1];
  float accel_z = acc_calibrated[2];
  ...
  if (gBlinkLedDate < millis ()) {
    ...
    frame.id = 0x00000018 ;
    frame.len = 8 ;
    float_to_bytes(gyro_x, frame.data);
    float_to_bytes(gyro_y, frame.data + 4);
    //    frame.data [0] =
    //    frame.data [1] =
    //    frame.data [2] =
    //    frame.data [3] =
